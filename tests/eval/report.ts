...
function pathIsValid(
  ...
  return path
    .slice(1)
    .every((kcId, index) => shortestPath(input.graph, path[index]!, kcId).length > 0);
}
...
export function evaluateFrozenCases(
  ...
  if (!suiteId.trim() || cases.length === 0)
    throw new Error('Evaluation suite needs an id and cases');
  ...
  const results = cases.map((testCase): EvaluationCaseResult => {
    ...
    const passed =
      actual.status === testCase.expectedStatus && actual.rootKcId === testCase.expectedRootKcId;
    const pathValid =
